...
import { analyzeReproductiveHealth, ReproHealthInsights } from '../../services/geminiService';
import InsightsIcon from '@mui/icons-material/Insights';
import PsychologyIcon from '@mui/icons-material/Psychology';
import SpaIcon from '@mui/icons-material/Spa';

interface ReproductiveHealthModuleProps {
  onBack: () => void;
}

const ReproductiveHealthModule: React.FC<ReproductiveHealthModuleProps> = ({ onBack }) => {
  const [cycleDay, setCycleDay] = useState('');
  const [symptoms, setSymptoms] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [insights, setInsights] = useState<ReproHealthInsights | null>(null);
  ...
  const handleGetInsights = async () => {
    ...
    setInsights(null);
    try {
      const result = await analyzeReproductiveHealth(day, symptoms);
      setInsights(result);
    } catch (err) {
      console.error(err);
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 2 }}>
      ...
      <Typography variant="h5" gutterBottom>Reproductive Health Insights</Typography>
            ...
            <Paper variant="outlined" sx={{p: 2}}>
                <List>
                    ...
                    <ListItem sx={{alignItems: 'flex-start'}}>
                        <ListItemIcon><PsychologyIcon color="primary" sx={{mt:1}} /></ListItemIcon>
                        <ListItemText 
                            primary="Symptom Explanations" 
                            secondary={
                                <Box component="ul" sx={{pl: 2, m: 0}}>
                                    {insights.symptom_explanations.map((exp, i) => <li key={i}>{exp}</li>)}
                                </Box>
                            } 
                        />
                    </ListItem>
                     <ListItem sx={{alignItems: 'flex-start'}}>
                        <ListItemIcon><SpaIcon color="primary" sx={{mt:1}} /></ListItemIcon>
                        <ListItemText 
                            primary="Wellness Tips" 
                             secondary={
                                <Box component="ul" sx={{pl: 2, m: 0}}>
                                    {insights.wellness_tips.map((tip, i) => <li key={i}>{tip}</li>)}
                                </Box>
                            } 
                        />
                    </ListItem>
...
export default ReproductiveHealthModule;